...
st.markdown("""<style>.stButton { background-color: #152e33; color: #d3e9ed; }</style>""", unsafe_allow_html=True)
...
st.markdown("<p style = 'text-align: center;font-size: 14px; font-style:italic'>Please note that PharmAssistant is not a substitute for professional medical advice. Always confirm with your pharmacist before purchasing an OTC medication!" \
...
st.markdown("<label style='color:#152e33; '>Describe your symptoms, health concerns, or requests:</label>", unsafe_allow_html=True)
...
with st.form(key="chatForm", clear_on_submit=True):
    chatInput = st.text_input("Describe your symptoms, health concerns, or requests:", key="chatInput")
