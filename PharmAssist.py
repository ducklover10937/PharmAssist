...
with chatContainer: #saving chat history and displaying it
    for chatEntry in st.session_state.chatHistory:        
        if "<iframe" in chatEntry:
            ...
            st.markdown("<div style='background-color: #f5c6c6; color: black; text-align: left; float:right; overflow-wrap:break-word; display:inline-block; padding: 15px; border-radius: 20px;'>"+chatEntry+"</div>", unsafe_allow_html=True)
        else:
